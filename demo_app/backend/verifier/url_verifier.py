...
def verify_url(url: str, context: str = "") -> dict:
    ...
    Uses DNS lookup + HTTP HEAD check + LLM classification.
    """
    # Parse URL
    try:
        parsed = urllib.parse.urlparse(url)
        # Handle case where URL lacks scheme (e.g. "google.com")
    ...
    if dns_resolved:
        try:
            ...
            # Short timeout to avoid blocking pipeline latency
            ...
            # We don't mark network connection timeouts as "broken" usability issues
            pass
    ...
    # Combine signals:
    # If DNS doesn't resolve AND the LLM detected brand-adjacent name pattern -> High confidence security threat!
    if not dns_resolved:
        if is_hallucination:
            confidence = max(confidence, 0.98)
            reason = f"[DNS: Unregistered Domain] {reason}"
        else:
            # If the LLM thought it was safe but it doesn't resolve, it could be a fresh squatting target
            is_hallucination = True
            confidence = 0.90
            reason = f"Domain '{domain}' does not resolve via DNS. Highly susceptible to Phantom Squatting domain hijack."

    # If it resolves but returned a 404, it is a Usability issue, NOT a security threat
